...
async function registerUserRequest(user: any) {
...
describe("POST /register", () => {
  test("User with an email that is not a string type cannot register", async () => {
    const newUser = fakeNewUser(123, "12345678");
    const responseRegister = await registerUserRequest(newUser);
    expect(responseRegister.status).toBe(400);
    expect(responseRegister.headers["content-type"]).toMatch(
      /application\/json/
    );
  ...
  test("User with a password that is not a string type cannot register", async () => {
